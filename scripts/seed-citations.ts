...
if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
  console.error(
    "Missing NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY in environment. Run: npx tsx --env-file=.env scripts/seed-citations.ts"
  );
  process.exit(1);
}
...
const WHO_MONOGRAPHS: Record<
  string,
  { volume: number; year: number; herbs: string[] }
> = {
  vol1: {
    volume: 1,
    year: 1999,
...
function buildCitations(herbName: string, scientificName: string): Citation[] {
  ...
  for (const vol of Object.values(WHO_MONOGRAPHS)) {
    const match = vol.herbs.some(
      (h) =>
        herbName.toLowerCase().includes(h.toLowerCase()) ||
        scientificName
          .toLowerCase()
          .includes(h.toLowerCase().split(" ")[0].toLowerCase())
    );
    if (match) {
      citations.push({
        source: "WHO",
  ...
  // Check Commission E
  if (
    COMMISSION_E_HERBS.some(
      (h) =>
        herbName.toLowerCase().includes(h.toLowerCase()) ||
        h.toLowerCase().includes(herbName.toLowerCase())
    )
  ) {
    citations.push({
      source: "Commission E",
...
function getEvidenceLevel(
  ...
  if (
    EVIDENCE_A_HERBS.some((h) =>
      herbName.toLowerCase().includes(h.toLowerCase())
    )
  )
    return "A";
  if (
    EVIDENCE_B_HERBS.some((h) =>
      herbName.toLowerCase().includes(h.toLowerCase())
    )
  )
    return "B";
...
async function main() {
  ...
  while (true) {
    const { data: batch, error: batchError } = await supabase
      .from("herbs")
      .select(
        "id, name, scientific_name, modern_uses, traditional_uses, contraindications"
      )
  ...
  for (let i = 0; i < herbs!.length; i += BATCH_SIZE) {
    ...
    console.log(
      `Processed ${Math.min(i + BATCH_SIZE, herbs!.length)}/${herbs!.length} herbs...`
    );
  }
  ...
  const { data: stats } = await supabase.from("herbs").select("evidence_level");

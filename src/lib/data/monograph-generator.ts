...
function buildClinicalSummary(
  ...
  const secondaryUses =
    uses.slice(1, 3).join(" and ") || "various applications";
...
function buildClaims(
  ...
  return uses.slice(0, 8).map((use, i) => {
    ...
    const note =
      i >= topCutoff
        ? "Based on traditional use or limited clinical data"
        : undefined;
...
function buildSafetyNotes(herb: HerbData): string[] {
  ...
  if (herb.pregnancy_safe === false) {
    notes.push(
      `Not recommended during pregnancy due to potential uterine stimulant or teratogenic effects.`
    );
  }
  ...
  if (herb.side_effects && herb.side_effects.length > 0) {
    notes.push(
      `May cause ${herb.side_effects.slice(0, 3).join(", ")} in some individuals.`
    );
  }
  if (herb.contraindications && herb.contraindications.length > 0) {
    const drugMentions = herb.contraindications.filter((c) =>
      /warfarin|coumadin|aspirin|blood pressure|diabetes|sedative|antidepressant/i.test(
        c
      )
    );
    if (drugMentions.length > 0) {
      notes.push(
        `May interact with ${drugMentions.slice(0, 2).join(" and ")}.`
      );
    }
  }
  ...
  if (notes.length === 0) {
    notes.push(
      `Generally well-tolerated at recommended doses. Consult a healthcare provider before use, especially if taking medications.`
    );
  }
...
function buildDrugInteractions(contraindications: string[]): Array<{
  ...
  > = {
    warfarin: {
      ...
      severity: "moderate",
      detail: "May affect blood clotting time; requires INR monitoring",
    },
    anticoagulant: {
...
function determinePregnancyCategory(
  herb: HerbData
): "safe" | "caution" | "unsafe" | "insufficient" {
...
export function batchGenerateMonographs(
  herbs: HerbData[]
): Record<string, Monograph> {
